    ...
    tokio::task::spawn_blocking(move || {
        loop {
            if let Ok(event) = crossterm::event::read()
                && tx_input.blocking_send(AppEvent::Input(event)).is_err()
            {
                break;
            }
        }
    ...
    tokio::spawn(async move {
        while let Some(query) = search_rx.recv().await {
            ...
            if latest_query.len() > 1
                && let Ok(results) =
                    crate::network::search_ticker(&client_search, &latest_query).await
            {
                let _ = tx_search_res
                    .send(AppEvent::SearchResultsFetched(results))
                    .await;
            }
        }
    });
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            match crate::network::fetch_market_status(&client_clone).await {
                Ok(status) => {
                    let _ = tx_clone.send(AppEvent::MarketFetched(Ok(status))).await;
                }
                Err(e) => {
                    let _ = tx_clone.send(AppEvent::MarketFetched(Err(e))).await;
                }
            }
        }
    });
            ...
            if let Some(event) = rx.recv().await {
                match event {
                    ...
                    AppEvent::SearchResultsFetched(results) => {
                        ...
                        app.search_state.select(if app.search_results.is_empty() {
                            None
                        } else {
                            Some(0)
                        });
...
pub fn ui(frame: &mut ratatui::Frame, app: &mut App) {
    ...
    if let Some(status) = &app.market_status {
        ...
        frame.render_widget(
            Paragraph::new("Loading Market Data...").style(Style::default().fg(Color::DarkGray)),
            main_layout[0],
        );
    }
    ...
    if let Some(details) = &app.details {
        ...
        let col1_text = vec![
            ...
            Line::from(vec![
                Span::styled("52W High: ", Style::default().fg(Color::Gray)),
                Span::styled(
                    format!("${:.2}", details.high_52w),
                    Style::default().fg(Color::Green),
                ),
            ]),
            Line::from(vec![
                Span::styled("52W Low:  ", Style::default().fg(Color::Gray)),
                Span::styled(
                    format!("${:.2}", details.low_52w),
        ...
        let col2_text = vec![
            Line::from(vec![
                Span::styled("Mkt Cap:  ", Style::default().fg(Color::Gray)),
                Span::raw(format!(
                    "${:.2}B",
                    details.market_cap as f64 / 1_000_000_000.0
                )),
            ]), // Billions
            Line::from(vec![
                Span::styled("P/E Ratio:", Style::default().fg(Color::Gray)),
                Span::raw(fmt_num(details.pe_ratio, "")),
            ]),
            Line::from(vec![
                Span::styled("Div Yield:", Style::default().fg(Color::Gray)),
                Span::raw(fmt_num(details.dividend_yield, "%")),
            ]),
        ];
        ...
        let col3_text = vec![
            Line::from(vec![
                Span::styled("YTD Ret:     ", Style::default().fg(Color::Gray)),
                Span::raw(fmt_num(details.year_return, "%")),
            ]),
            Line::from(vec![
    ...
    let status = Paragraph::new(app.message.as_str()).style(Style::default().fg(app.message_color));
